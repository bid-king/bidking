...
export function BidPrice({ align = 'center' }: Props) {
  // const [priceArr, prev, curr, err] = useBidPrice();
  const [priceArr, setPriceArr] = useState<string[]>(['0']);
  useEffect(() => {
    setPriceArr(bidPriceParse('101293').split('')); //중간에 테스트할 숫자를 입력하고 테스트하면됩니다
  }, [priceArr]);
  //여기까지를 주석 처리해야합니다.
  return (
    <div css={{ width: '100%', height: '3.25rem', position: 'relative' }}>
      <div
        css={{
          ...
          width: `${
            1.5 +
            1.8 * (priceArr.length - (Math.floor(priceArr.length / 3) - 1)) +
            1.15 * (Math.floor(priceArr.length / 3) - 1)
          }rem`,
